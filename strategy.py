...
- Yeni: reconcile_with_exchange() - bot calisirken RECONCILE_INTERVAL_SECONDS'ta
  (varsayilan 60sn) bir, botun kendi acik pozisyon kayitlarini borsayla
  IKI YONLU karsilastirir (bkz. fonksiyonun kendi docstring'i).
...
# ------------------------------------------------------------
def _build_position_from_exchange(p: dict) -> state_module.Position:
    ...
    lose exit borsada gercek bir emir olarak durmadigi icin bu SL'den
    sabit orantiyla GERI HESAPLANIR (bkz. state.reconstruct_lose_exit).
    Hem baslangictaki restart-reconcile'i hem de calisirken periyodik
    olarak calisan iki-yonlu reconcile'i besler."""
    ...
    return state_module.Position(
        symbol=p["symbol"], side=side, position_idx=pos_idx,
...
def _exchange_positions_by_symbol(client) -> dict:
    """Borsadaki acik pozisyonlari, takip edilen coin listesiyle
    sinirlandirip sembol -> pozisyon sozlugune cevirir. Ayni sembolde
    birden fazla pozisyon bulunursa (ornegin elle acilmis bir hedge
    kalintisi) sadece ilkini alir ve durumu loglar."""
    ...
    for p in exchange_positions:
        ...
        if symbol in by_symbol:
            print(f"[reconcile] {symbol}: borsada birden fazla acik pozisyon bulundu, "
...
def reconcile_open_positions(client, bot_state):
    ...
    for symbol, p in _exchange_positions_by_symbol(client).items():
        pos = _build_position_from_exchange(p)
        if pos.sl_price <= 0:
            print(f"[reconcile] {symbol} {pos.side}: SL bulunamadi, sadece giris takip edilecek")
        bot_state.add_position(pos)
        notify.notify_position_found_on_restart(
            symbol, pos.side, pos.entry_price, pos.sl_price, pos.lose_exit_price, pos.leverage)
...
def reconcile_with_exchange(client, bot_state):
    ...
    YONLU karsilastirir:
      ...
      1) Bot'ta acik gorunen ama borsada olmayan bir pozisyon varsa,
         ...
         islenir (bu, bot disinda bir pozisyonu kapatabilecek tek
         mekanizma oldugu icin en dogru varsayimdir).

      2) Borsada acik olan ama bot'un bilmedigi bir pozisyon varsa
         (ornegin borsa arayuzunden elle acilmis, ya da bir aksama
         sonucu bot'un kaydetmeyi kacirdigi bir islem), gereken tum
         hesaplamalar (lose exit, ayrilan miktar) yapilarak kayda
         alinir - tipki restart'ta oldugu gibi, ama bot CALISIRKEN.

    NOT: Bu fonksiyon eski reconcile_externally_closed'in yerini alir -
    onun yaptigi 1. yon islemini de kapsar, ayrica 2. yonu de ekler."""
    exchange_by_symbol = _exchange_positions_by_symbol(client)

    # 1) Bot'ta var, borsada yok -> disaridan kapanmis, kayittan sil
    for pos in list(bot_state.all_positions()):
        if pos.symbol in exchange_by_symbol:
            continue
    ...
    for symbol, p in exchange_by_symbol.items():
