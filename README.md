...
**Borsa ile kayıt senkronizasyonu:** Bot, açık pozisyon kayıtlarını
her ~60 saniyede bir (`RECONCILE_INTERVAL_SECONDS`) borsayla **iki
yönlü** karşılaştırır:
- Bot'ta açık görünen ama borsada olmayan bir pozisyon varsa, dışarıdan
  (gerçek SL tetiklenmesi gibi) kapandığı varsayılıp kayıttan silinir.
- Borsada açık olan ama bot'un bilmediği bir pozisyon varsa (elle
  açılmış olabilir, ya da bir aksama sonucu bot kaydetmeyi kaçırmış
  olabilir), gereken hesaplamalar (özellikle lose exit'in güvenlik
  SL'den geri hesaplanması) yapılarak kayda alınır ve bunun için ayrı
  bir Telegram bildirimi gider — bu bildirimi görürsen o pozisyonu bir
...
kayıtsız pozisyon (bkz. yukarıdaki "kayıt senkronizasyonu").
