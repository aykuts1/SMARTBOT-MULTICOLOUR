...
- Anlık bildirimler (modern tasarım, madde 48-50)
- 3 rapor: hourly, 12h Z, 24h X (madde 51-53)
- Komut polling (long polling, madde 54)
- Flag bildirimleri ARTIK ATILMIYOR — sadece raporlarda görünür
"""
import json
import logging
import queue
...
class TelegramThread(threading.Thread):
    def __init__(self, config, data_manager=None, trade_manager_ref=None, control_ref=None):
        ...
        self.ctrl = control_ref  # Bot ana kontrolü (start/stop trading)
        ...
        # Bot başlama anı (raporlar için "ne kadar süredir aktif" hesabı)
        self.bot_start_ts = now_ts()

    # ------------------------------------------------------------------
    # SETTERS (Bot init sırasında circular ref'leri doldurmak için)
    # ------------------------------------------------------------------
    ...
    def _send_now(self, text):
        """Kuyruksuz doğrudan gönderim (sender thread içinden çağrılır)."""
    ...
    # ==================================================================
    # ANLIK BİLDİRİMLER (modern tasarım)
    # ==================================================================
    ...
    def notify_trade_open(self, trade, hard_sl=None):
        ...
        thread_icon = {"RED": "🔴", "BLUE": "🔵", "YELLOW": "🟡"}.get(trade.thread, "")
        lines = [f"{thread_icon} <b>İŞLEM AÇILDI</b> {emoji}", ""]
    ...
    def notify_thread_ready(self, red_trade, thread, side, level_lines):
        thread_icon = {"BLUE": "🔵", "YELLOW": "🟡"}.get(thread, "")
        thread_name = "MAVİ" if thread == "BLUE" else "SARI"
        lines = [f"{thread_icon} <b>{thread_name} TABLO HAZIR</b>", ""]
        lines.append(f"📊 {red_trade.symbol} | {side} | parent: 🔴 {red_trade.side}")
        lines.append("")
    ...
    # ==================================================================
    # AÇIK İŞLEMLER & FLAGLER BLOĞU (raporlarda kullanılır)
    # ==================================================================
    def _build_open_trades_block(self):
        ...
        lines = []
        for t in sorted(open_trades, key=lambda x: (x.symbol, x.thread)):
            icon = {"RED": "🔴", "BLUE": "🔵", "YELLOW": "🟡"}.get(t.thread, "")
    ...
    def _build_open_flags_block(self):
        ...
        if self.ctrl:
            try:
                flags += self.ctrl.red_thread.get_open_flags()
            except Exception:
                pass
            try:
                flags += self.ctrl.blue_thread.get_open_flags()
            except Exception:
                pass
            try:
                flags += self.ctrl.yellow_thread.get_open_flags()
            except Exception:
                pass
        if not flags:
        ...
        lines = []
        for f in flags:
            icon = {"RED": "🔴", "BLUE": "🔵", "YELLOW": "🟡"}.get(f["thread"], "")
    ...
    def _build_stats(self, trades):
        """Verilen trade listesi için temel istatistikler."""
        if not trades:
            return None
        total = len(trades)
    ...
    def send_hourly_report(self):
        """1 saatlik klasik durum raporu (madde 51)."""
        now = now_ts()
        start = now - 3600

        bal = self.dm.get_balance() if self.dm else 0.0
        stake = self.tm.get_stake() if self.tm else 0.0
        counts = self.tm.slots.count_by_thread() if self.tm else {"RED": 0, "BLUE": 0, "YELLOW": 0}
        ...
        lines.append(f"💰 Bakiye: <b>{fmt_money(bal)} USDT</b>")
        lines.append(f"🎯 Stake: <b>{fmt_money(stake)} USDT</b>")
        lines.append(f"📌 Açık: 🔴 {counts.get('RED', 0)} | 🔵 {counts.get('BLUE', 0)} | 🟡 {counts.get('YELLOW', 0)}")
        if paused:
    ...
    def send_12h_report(self):
        ...
        for thr in ("RED", "BLUE", "YELLOW"):
            tt = [t for t in closed if t.thread == thr]
            if not tt:
                continue
            s = self._build_stats(tt)
            icon = {"RED": "🔴", "BLUE": "🔵", "YELLOW": "🟡"}[thr]
            lines.append(f"{icon} {thr}: {s['total']} işlem | WR {s['winrate']:.0f}% | "
        ...
        for t in closed:
            # Exit name'in son kelimesini al (WINRATE / LOSE / EXIT vs)
            key = t.exit_name or "UNKNOWN"
            # Kategorize et
            if "WINRATE" in key:
                cat = "WINRATE"
            elif "LOSE" in key:
                cat = "LOSE"
            elif "CHANDELIER" in key:
                cat = "CHANDELIER"
            elif "KIRMIZI KAPANDI" in key:
                cat = "BAĞIMLI"
            else:
                cat = "TRAIL/DİĞER"
    ...
    def send_24h_report(self):
        """24 saatlik X raporu — çok detaylı (madde 53)."""
        now = now_ts()
        start = now - 24 * 3600
        closed = self.tm.get_closed_trades_window(start, now) if self.tm else []
        ...
        for thr in ("RED", "BLUE", "YELLOW"):
            ...
            s = self._build_stats(tt)
            icon = {"RED": "🔴", "BLUE": "🔵", "YELLOW": "🟡"}[thr]
            lines.append(f"{icon} {thr}: {s['total']} | WR {s['winrate']:.0f}% | Net {fmt_money(s['net'])}")
        lines.append("")
    ...
    def _check_reports(self):
        ...
        # 12 saatlik: 00 ve 12 saatlerinde
    ...
    def run(self):
        ...
        while not self._stop.is_set():
            ...
            # Long polling zaten ~25 sn'lik blok yapıyor, ek sleep küçük
            time.sleep(1)
