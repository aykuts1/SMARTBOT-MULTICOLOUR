...
def compute_all(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mum verisi. Geri donen dataframe'e su kolonlar eklenir:
        ...
        long_entry_line, long_exit_line, short_entry_line, short_exit_line
    ...
    bunlar GIRIS sinyali icin kullanilir (hangi yonde giris yapilacagini
    o anki trend belirledigi icin dogru secim budur).

    long_exit_line / short_exit_line: "up" ve "dn" cizgileri HER ZAMAN,
    trend hangi yonde olursa olsun, paralel olarak hesaplanir (Supertrend'in
    kendi ic mantigi boyle calisir). Bu yuzden ACIK BIR POZISYONUN TP
    seviyesi kontrol edilirken, o anki genel "trend" bayragi degil,
    pozisyonun KENDI yonune karsilik gelen bu sabit-tarafli kolonlar
    kullanilmalidir (bkz. strategy.check_exit). Aksi halde, mum henuz
    kapanmadan (saniyelik veriyle) genel trend gecici olarak ters
    donerse, exit_line de karsi tarafin formulune gecer ve pozisyonun
    yonuyle uyusmayan - fiyatin zaten gecmis oldugu - bir seviye haline
    gelebilir; bu da zararli bir kapanisin yanlislikla "Take Profit"
    olarak etiketlenmesine yol acar (canli ortamda tam olarak bu
    yasandi ve bu yuzden ayri sabit-tarafli kolonlar eklendi).
    """
    out = df.copy()
    ...
    out["long_entry_line"] = out["supertrend_up"] + config.ENTRY_LINE_ATR_MULT * out["atr_deger"]
    out["long_exit_line"] = out["supertrend_up"] + config.EXIT_LINE_ATR_MULT * out["atr_deger"]
    out["short_entry_line"] = out["supertrend_dn"] - config.ENTRY_LINE_ATR_MULT * out["atr_deger"]
    out["short_exit_line"] = out["supertrend_dn"] - config.EXIT_LINE_ATR_MULT * out["atr_deger"]

    is_up = out["trend"] == 1
    out["entry_line"] = np.where(is_up, out["long_entry_line"], out["short_entry_line"])
    out["exit_line"] = np.where(is_up, out["long_exit_line"], out["short_exit_line"])
