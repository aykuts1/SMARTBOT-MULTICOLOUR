...
def open_position(exchange, state, cfg, symbol: str, side: str,
    ...
    order = exchange.place_market_order(
        ...
        stop_loss_price=_compute_loss_exit_price(price, side, cfg["loss_exit_pct"]),
    )
...
def close_position(exchange, state, symbol: str, reason: str,
    ...
    pnl_usdt = position["margin_usdt"] * (pnl_pct / 100.0) * _leverage_from_position(position)
...
def _leverage_from_position(position: dict) -> float:
    ...
    if position["margin_usdt"] == 0:
        return 1.0
    return notional / position["margin_usdt"]
